...
# Target hariç
X_fs = df.drop(columns=["Machine failure"])
y_fs = df["Machine failure"]
...
print("\nÖzellik Önem Sıralaması (RF):")
print(feature_importance_df)

# İlk 5 özelliği al- 10 özellik denendi ancak anlamlı fark olmadı.
top_k = 5
...
print(f"\nEn önemli ilk {top_k} özellik:")
for f in top_features["Feature"]:
    print(f" - {f}")

# ve kaydet
feature_importance_df.to_csv(
    os.path.join(OUTPUT_DIR, "feature_importance_rf.csv"),
    index=False
)

top_features.to_markdown(
    os.path.join(OUTPUT_DIR, "feature_selection_top5.md"),
    index=False
)

print("Feature selection çıktıları kaydedildi.")
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
    ...
    for name, model in models.items():
        ...
        # TRAINING TIME - sadece trainde fit boylece sızıntı yok
